...
class VariablesAdapterNew {
    ...
    protected function fillContent($content)
    {
        ...
        foreach ( (array) $matches[1] as $index => $match) {
            ...
            $name       = $this->findExtensionName($component);
            $variables  = (array) Arr::get($notifications, $name . '.variables', []);
     ...
     * @return mixed
     */
    protected function resolveValue($name, array $variables)
    {
        ...
        if (isset($variable['dataProvider'])) {
            $value = $this->resolveDataProviderValue($variables['dataProvider']);
            if (!$value) {
                return false;
            }
            return $value;
